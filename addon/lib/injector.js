...
// modified by Ben to present a simpler interface
// this was easier to do by making things jetpack specific.
var { Cc, Ci, Cm, Cu, Cr, components } = require("chrome");
const xulApp = require("api-utils/xul-app");
const unload = require("unload");

var HAS_NAVIGATOR_INJECTOR = xulApp.versionInRange(xulApp.version, "9.0a2", "*");
...
var gInjector=null;
function init() {
  if (!HAS_NAVIGATOR_INJECTOR && !gInjector)
...
// register the API
exports.registerNavigatorAPI = function(name, apiObject) {
  ...
  NavigatorAPI.prototype = {
    ...
    init: function API_init(aWindow) {
      ...
      let apiInstance = apiObject(aWindow);
      
      function genPropDesc(val) {
        return {enumerable: true, configurable: true, writable: false,value: val};
      }

      let properties = {};
      for (var fn in apiInstance) {
        properties[fn] = genPropDesc(apiInstance[fn]);
      }
  ...
  MozIDAPIContract = "@mozilla.org/vep/id;1";
  MozIDAPIClassID = Components.ID("{3f4d6c24-dc28-48e6-ba1b-2d9f03270ab2}");
  function MozIDAPI() {}
  MozIDAPI.prototype = {
    __proto__: NavigatorAPI.prototype,
    classID: MozIDAPIClassID
  };
  
  let MozIDAPIFactory = {
    createInstance: function(outer, iid) {
      if (outer != null) throw Cr.NS_ERROR_NO_AGGREGATION;
      return new MozIDAPI().QueryInterface(iid);
    }
  };
  
  // register our navigator api's that will be globally attached
  Cm.QueryInterface(Ci.nsIComponentRegistrar).registerFactory(
    MozIDAPIClassID, "MozIDAPI", MozIDAPIContract, MozIDAPIFactory
  );
  Cc["@mozilla.org/categorymanager;1"].getService(Ci.nsICategoryManager).
    addCategoryEntry("JavaScript-navigator-property", name,
                     MozIDAPIContract,
                     false, true);
  
  unloaders.push(function() {
    Cm.QueryInterface(Ci.nsIComponentRegistrar).unregisterFactory(
      MozIDAPIClassID, MozIDAPIFactory
    );
    Cc["@mozilla.org/categorymanager;1"].getService(Ci.nsICategoryManager).
      deleteCategoryEntry("JavaScript-navigator-property", "id", false);
  });
